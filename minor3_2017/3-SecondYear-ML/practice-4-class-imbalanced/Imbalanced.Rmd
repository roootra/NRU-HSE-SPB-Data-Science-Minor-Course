...
На прошлом занятии вы работали с набором данных Caravan, при этом если вы не предпринимали никаких специальных действий, то вы легко получали очень "хорошую" модель с высоким accuracy.

Давайте попробуем вместе.

Начнем с разбиения на тестовую и обучающую выборки и построим модель классификации на основе дерева решений ctree.
...
tree_model_weighted <- train(Purchase~., method = 'ctree', data = CaravanTrain, trControl=cv5, weights = model_weights)
...
К сожалению, мы заметно потеряли в accurasy, да и Sensitivity (Recall) заметно упал. Возможно мы переобучились и стоит понастраивать вес как параметр модели. Но в любом случае, такая модель лучше исходной, так как исходная вообще почти не детектировала положительые случаи. 
...
tree_model_down <- train(Purchase~., method = 'ctree', data = CaravanTrain, trControl=cv5_down)
...
А тут все стало сильно хуже. По сравнению с модельюгде мы использовали веса, совсем плохо и это на обучающей выборке. А почему так?
...
cv5_up<-cv5
cv5_up$sampling<-"up"
tree_model_up <- train(Purchase~., method = 'ctree', data = CaravanTrain, trControl=cv5_up)
...
На обучаещей выборке модель ведет себя прекрасно. А на тесте?
...
tree_model_smote <- train(Purchase~., method = 'ctree', data = CaravanTrain, trControl=cv5_smote)
...
```{r}
library(readr)
blood <- read_csv("~/shared/minor3_2017/3-SecondYear-ML/practice-4-class-imbalanced/blood.csv")
table(blood$`whether he/she donated blood in March 2007`)
#library(dplyr)
#blood = rename(blood, donation = `whether he/she donated blood in March 2007`)
