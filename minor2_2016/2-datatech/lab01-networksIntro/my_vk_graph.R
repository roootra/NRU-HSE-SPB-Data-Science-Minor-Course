...
#  code = "03cbd66759ac8384f3"
  ...
  #users.get(vk, user_id = "avsirotkin")
  friends.list<-vk(method="friends.get")
  ...
  while (rest > bunch_size){
    ...
    friends.info.sub<-users.get(vk,id.list)
  ...
  for (i in friends.list){
    mutual.list<-vk(method="friends.getMutual", target_uid = i)
    Sys.sleep(.5)
